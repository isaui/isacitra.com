...
export default function () {
    ...
    useEffect(() => {
        const setup = async () => {
            if(id){
                try {
                    axios.get(`http://localhost:3001/articles/${id}`).then(result => {
                        const {post} = result.data
                        console.log(post)
                        setTitle(post.title)
                        setCategories(post.categories)
                        setHtmlText(post.content)
                ...
                try {
                    const res = await axios.get('http://localhost:3001/articles/new')
    ...
    const handleDelete = async (articleId) => {
        try {
                if(isReady){
                    axios.delete('http://localhost:3001/articles/'+articleId).then((res)=>{
                navigate(-1)
            })
                }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        await axios.post('http://localhost:3001/articles/', { "title": title, "content":updatedHtmlString, "tableOfContents": JSON.stringify(toc),
        ...
        <div className=' min-h-screen flex justify-center flex-col w-full'>
         ...
         <form  onSubmit={handleSubmit} className=" max-w-[1024px] flex flex-col  mx-auto">
            <div className="my-2 mx-4 flex justify-between items-center">
                <h1 className="text-white text-2xl md:text-4xl">{isUpdate? 'Edit' : 'Create New'} Post</h1>
            ...
            <input value={title} onChange={e => setTitle(e.target.value)}type="text" id="title" class=" bg-slate-800 focus:border  text-white  text-sm rounded-xs focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " placeholder="Hal Menarik di Kampus Saya"></input>
        </div>
        <div className=" my-2 mx-4">
